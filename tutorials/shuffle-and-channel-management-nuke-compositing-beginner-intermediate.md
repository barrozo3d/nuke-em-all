...
- [After Effects to Nuke: 1 Hour FREE Course | Compositing in Nuke](after-effects-to-nuke-1-hour-free-course-compositing-in-nuke.md) — shares the beginner-onboarding-to-Nuke's-node-graph angle; that course covers the layers-vs-nodes, Roto/masking, and merge-operations fundamentals that come before this video's channel/Shuffle-specific deep dive.
- [RenderOutputDefine](renderoutputdefine.md) — shares `channels` + `aovs`; the render-side counterpart — its `channel` parameter (and any PrmanOutputChannelDefine channels) decide which channels exist at all before Shuffle decides what to do with them.
- Intro to Nuke for 3D Artists - Full VFX Course (`intro-to-nuke-for-3d-artists---full-vfx-course.md`) — a full 4h+ course that teaches this video's Shuffle/AOV-splitting fundamentals from scratch as part of a complete beauty-recreation pipeline, then goes further into Cryptomatte, STmap, and in-comp relighting on the same footage.
