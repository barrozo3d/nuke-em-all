...
- [Multiple NetworkMaterials with NetworkMaterialCreate](multiple-networkmaterials-with-networkmaterialcreate.md) — shares `katana` + `lookdev` + `nodegraph`; this page wires one NetworkMaterialCreate to a single `usdSurface` terminal — that one generalises it to several NetworkMaterials sharing one network.
- [Loading USD Plug-ins into Katana](loading-usd-plug-ins-into-katana.md) — shares `katana` + `usd` + `katana-9`; **this page's stated prerequisite.** It also adds the `usd` menu to the NetworkMaterialCreate terminal sidebar, which is where the UsdPreviewSurface here is reached.
