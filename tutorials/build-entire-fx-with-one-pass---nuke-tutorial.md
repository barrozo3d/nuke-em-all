...
- Stormy Crystal Skull | Nuke, Blender, Ai, Embergen, Mixed VFX Medium Part 2 (`stormy-crystal-skull-nuke-blender-ai-embergen-mixed-vfx-medium-part-2.md`) — shares `compositing`, `merge`, `channels` and the same "isolate a region via a position/ID pass, then multiply/mask to fake extra CG detail entirely in comp" philosophy, plus expression-driven Grade flicker/animation.
- Nuke Compositing an Advanced CG Shockwave | VFX (LookDev) (`nuke-compositing-an-advanced-cg-shockwave-vfx-lookdev.md`) — shares the underlying philosophy of driving comp-side FX from position/UV passes and Cryptomatte-isolated regions rather than re-rendering, at a much more advanced/kitbashed scale.
- Compositing EPIC VFX Godrays | Nuke Tutorial (`compositing-epic-vfx-godrays-nuke-tutorial.md`) — shares the position-pass-driven `noise()` expression technique, there used to build a ground-interactive God Ray matte instead of surface texture breakup.
