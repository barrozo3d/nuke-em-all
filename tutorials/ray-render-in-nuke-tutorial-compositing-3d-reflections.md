...
- Compositing in UV space with Projections | Nuke [Advanced] (`compositing-in-uv-space-with-projections-nuke-advanced.md`) — shares `3d-system`, and the same `RayRender`/`AmbientOcclusion` nodes used there for dynamic relighting on deforming geometry.
- Nuke Compositing Technique | Card3D + PixelsToPos [Beginners] (`nuke-compositing-technique-card3d-pixelstopos-beginners.md`) — shares `3d-system`, `camera-tracking`; a lighter-weight beginner alternative for sticking 2D elements/corrections to a tracked scene, without full proxy geometry.
