...
- [Multiple NetworkMaterials with NetworkMaterialCreate](multiple-networkmaterials-with-networkmaterialcreate.md) — shares `katana` + `lookdev` + `nodegraph`; the multi-material case this page's Material Scenegraph and Namespaces exist for — its worked example cuts 27 nodes to 19 by sharing 8 across two robot material variants.
