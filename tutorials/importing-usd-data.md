...
- [Composition Arcs - Understanding LIVRPS](composition-arcs---understanding-livrps.md) — shares `katana` + `usd` + `scenegraph`; **the ordering behind every choice on this page.** It picks an arc by purpose; that one says which arc *wins* when several describe the same property.
