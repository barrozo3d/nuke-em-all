...
- Nuke Compositing Technique | Card3D + PixelsToPos [Beginners] (`nuke-compositing-technique-card3d-pixelstopos-beginners.md`) — this beginner-level video explicitly references this tutorial's ST-map/projection technique as the "more advanced" way of sticking corrections to CG scenes, offering a Card3D-gizmo shortcut instead.
- Parallax HAX | Nuke Compositing [Advanced] (`parallax-hax-nuke-compositing-advanced.md`) — explicitly references this tutorial's UV-coordinate-system/ST-map technique as a prerequisite before building its 2D-only fake-parallax planes.
- UVs and UV Passes in Nuke: PART 1 [Beginner] (`uvs-and-uv-passes-in-nuke-part-1-beginner.md`) — the beginner-tier fundamentals series this tutorial builds toward; covers what a UV pass and `STMap` actually are before this video applies them to full projection/relighting workflows.
- UV / ST Maps [Part 2] | Nuke Compositing [Beginner / Intermediate] (`uv-st-maps-part-2-nuke-compositing-beginner-intermediate.md`) — the direct intermediate-tier precursor to this tutorial's workflow: projecting an expression-based UV coordinate pattern onto 3D geometry, precomping it, and using `STMap` for sticky roto/grading — this tutorial extends the same core idea into full `ScanlineRender`-based UV-space compositing.
