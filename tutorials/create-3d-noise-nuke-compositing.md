...
- Shuffle and Channel Management | Nuke Compositing [Beginner / Intermediate] (`shuffle-and-channel-management-nuke-compositing-beginner-intermediate.md`) — shares `channels`, `aovs`; that video explains the layer/Shuffle fundamentals for pulling a CG render's stored position/utility passes out to work with, exactly what this tutorial's technique depends on.
- Compositing EPIC VFX Godrays | Nuke Tutorial (`compositing-epic-vfx-godrays-nuke-tutorial.md`) — shares the position-pass-driven `noise()` expression technique, applied there to build a ground-interactive God Ray matte instead of surface texture breakup.
