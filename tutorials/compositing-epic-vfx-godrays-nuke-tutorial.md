...
- The BLUEPRINT for Cinematic Light (VFX) (`the-blueprint-for-cinematic-light-vfx.md`) — shares the position-pass-driven procedural-noise technique (there: `P_NoiseAdvanced` gizmo masked by existing highlights; here: raw `noise()` expression), both adding surface-space detail from a CG render's position pass.
