...
- **Summary:** Katana scene graph fundamentals - the orientation page the rest of this skill's Katana entries assume. The scene graph is a VIEW of the scene generated by the recipe up to the node holding the blue square (the view node), set with Edit > View Selected Node or the V key. Defines parent, child, sibling, leaf and branch against /root, /root/world and /root/materials, the per-location type attribute, the read-only Attributes tab, and implicit resolvers - the deferred material and texture copies that keep everything editable at a high level and speed up scene generation.
- **File:** tutorials/using-the-scene-graph.md
- **Related:** What is Katana? (`what-is-katana.md`), Using the Scene Explorer (`using-the-scene-explorer.md`) — share `katana`, `scenegraph`; Material Basics (`material-basics.md`) — the implicit resolver acting on materialAssign.
...
- **Summary:** The Scene Explorer tab - USD Scene Graph and Katana Scene Graph in one panel, plus Collections. USD data columns Name, Type, Kind (component/assembly, set with UsdKindSet) and Purpose (default/proxy/render, with proxy prims arriving render-invisible), shared working sets for viewer visibility, render inclusion and live render updates, and two USD-only columns for payloads and prim activation. The useful part is the styling vocabulary for debugging composition: blue branch defined, purple branch abstract class, dotted lines instance proxy, dimmed undefined over, dimmed strike-through inactive. USD locations cannot be deleted - UsdActiveSet deactivation persists, the working-set toggle is temporary.
- **File:** tutorials/using-the-scene-explorer.md
- **Related:** Using the Scene Graph (`using-the-scene-graph.md`) — shares `katana`, `scenegraph`; the Katana graph this panel embeds. Loading USD Plug-ins into Katana (`loading-usd-plug-ins-into-katana.md`) — shares `katana`, `usd`.
...
- **Summary:** Katana's design rationale: scene data described as a TREE OF FILTERS handed to the renderer as one recursive procedural and evaluated lazily, per location, through scene graph iterators - so a large scene graph state never has to be held in memory and the calling process keeps its own state. Covers what filters can do (Alembic import, shader instances, cameras and lights, transforms, rule-based material assignment, pass isolation, AOVs, Python attribute manipulation), the fallback for renderers without deferred procedurals (write a scene description file, losing the benefits and potentially very large), and compound nodes.
- **File:** tutorials/what-is-katana.md
- **Related:** Using the Scene Graph (`using-the-scene-graph.md`) — shares `katana`, `scenegraph`; the same model from the operator's side. OpScript Tutorials (`opscript-tutorials.md`) — the Python/Lua manipulation described here in the abstract.
...
- **Summary:** The LEGACY Katana material workflow (superseded by NetworkMaterialCreate as of 3.2) and still the page that defines the model: a Material node creates a material location under /root/materials or a nested namespace, shaders are attached with Add shader, and MaterialAssign binds it to geometry via CEL and the materialAssign attribute that an implicit resolver acts on at render time. Distinguishes edit (one at a time), override (geometry-specific, stored on materialOverride, many at once) and assign; covers shaders from multiple renderers on one location, MaterialResolve for early resolution, and FaceSetCreate for assigning different materials to parts of one mesh.
- **File:** tutorials/material-basics.md
- **Related:** Building Materials Using NetworkMaterialCreate (`building-materials-using-networkmaterialcreate.md`) — the workflow that supersedes this one. Adding Multiple Materials (`adding-multiple-materials.md`), GafferThree (`gafferthree.md`) — share `katana`, `lookdev`, `cel`.
...
- **Version:** 9.0 (learn.foundry.com/katana current docs at ingest; release notes whats_new_9.0)
- **Tags:** katana, lookdev, nodegraph, katana-9, intermediate
- **Summary:** NetworkMaterialCreate - the container node for the modern Katana material workflow, Group-like but holding only a material network, with a left-to-right shading node layout and support for multiple NetworkMaterial locations in one node. The hub page that names the five child topics (creating shading networks, multiple NetworkMaterials, ShadingGroup, Node Parameters and Interface Controls, NetworkMaterialEdit). Key practical fact: the workflow is forward AND backward compatible by copy-and-paste - old networks pasted in redraw in the new design correctly connected, and new shading nodes paste back out to older versions.
- **File:** tutorials/building-materials-using-networkmaterialcreate.md
- **Related:** Creating Shading Networks (`creating-shading-networks.md`), Node Parameters and Interface Controls (`node-parameters-and-interface-controls.md`), Organizing Shading Networks with ShadingGroup Nodes (`organizing-shading-networks-with-shadinggroup-nodes.md`), Editing Materials With The NetworkMaterialEdit Node (`editing-materials-with-the-networkmaterialedit-node.md`) — share `katana`, `lookdev`, `nodegraph`; the child topics of this hub page.
...
- **Summary:** The MaterialStack node - many materials in one node instead of a chain of Material nodes cluttering the recipe. Add Material, Add Look File Material, and Add Child Material for nesting (the parent must be inside the MaterialStack or the option is unavailable). Existing Material nodes are pulled in by Shift+middle-click dragging from the Node Graph; entries are duplicated, reordered by middle-click drag, disabled with Ignore Material (D) or removed with Delete Material.
- **File:** tutorials/adding-multiple-materials.md
- **Related:** Material Basics (`material-basics.md`) — shares `katana`, `lookdev`; supplies the shader-attachment steps this page defers to. Building Materials Using NetworkMaterialCreate (`building-materials-using-networkmaterialcreate.md`) — the modern equivalent.
...
- **Summary:** How to start a Katana render: Preview Render at any 3D node (Ctrl+P, static, never written to disk), Live Render (Ctrl+Shift+P, after opting objects in via the Live Render Updates column), and Disk Render from a Render node with RenderOutputDefine above it - without one the output lands in the temp directory. Multiple Preview Renders run simultaneously with no Katana-imposed limit, useful for rendering several camera angles at once. Cancel with Esc / Shift+Esc, repeat with Ctrl+backslash. Also documents Nuke Bridge and its three modes: Preview Comp, Live Comp and Interactive Comp.
- **File:** tutorials/performing-a-render.md
- **Related:** Render Types (`render-types.md`), Controlling Live Rendering (`controlling-live-rendering.md`) — share `katana`, `lighting`. RenderOutputDefine (`renderoutputdefine.md`) — the node this page says to place above the Render node.
...
- **Summary:** Katana's context-sensitive render options - renderer plug-ins advertise the methods they support, so the right-click menu varies by node and plug-in. Preview Render, Live Render, Preview Render with Profiling (captures Ops run, CPU cooking locations, memory for attributes and Lua scripts), Disk Render, Disk Render with Dependencies, Render Dependencies Only, plus farm plug-ins and the bundled Katana Queue. Two details worth having: starting a new Live Render automatically stops the previous one, and motion blur in Live Rendering needs the camera's makeInteractive set to No. Also Disk Render Dependencies and Disk Render Upstream Render Outputs, and a Debugging submenu that opens the traversal filters in a text editor.
- **File:** tutorials/render-types.md
- **Related:** Performing a Render (`performing-a-render.md`), Controlling Live Rendering (`controlling-live-rendering.md`) — share `katana`, `lighting`.
...
- **Version:** 9.0 (learn.foundry.com/katana current docs at ingest; release notes whats_new_9.0)
- **Tags:** katana, lighting, katana-9, intermediate
- **Summary:** Controlling Katana Live Rendering: it is opt-in per object via the Live Render Updates column in the Scene Graph tab, so only ticked objects restart the render. 3D Update Mode (Monitor tab, Scene Graph tab or menu bar) controls which material and light edits trigger a Live Render and when updates happen; the Viewer tab chooses the render viewpoint. Two caveats that look like bugs otherwise: adding new geometry such as PrimitiveCreate has NO effect because the render plug-ins do not support it, and with 3D Update Mode set to Manual, view node changes in the Node Graph are not reflected in the Scene Graph tab.
- **File:** tutorials/controlling-live-rendering.md
- **Related:** Performing a Render (`performing-a-render.md`), Render Types (`render-types.md`) — share `katana`, `lighting`.
...
- **Summary:** Writing custom OpenEXR metadata from a Katana disk render, with exact syntax. Portable route (Arnold AND RenderMan): an AttributeSet node at /root setting renderSettings.metadata.<attrib>.[key|value|prefix] - key or value must be present or nothing is written, and prefix decides whether the property reads exr/katana:key = value or exr/<prefix>:katana:key = value. RenderMan-only alternative: an OpScript at /root building attributes under renderSettings.outputs.primary.rendererSettings.exrheaders.* with Interface.SetAttr and StringAttribute / IntAttribute / FloatAttribute, scalar and array.
- **File:** tutorials/writing-exr-metadata-from-katana.md
- **Related:** OpScript Tutorials (`opscript-tutorials.md`) — shares `katana`, `python-scripting`; the Interface API used here. RenderOutputDefine (`renderoutputdefine.md`), Performing a Render (`performing-a-render.md`) — share `katana`.
...
- **Version:** 9.0 (learn.foundry.com/katana current docs at ingest; release notes whats_new_9.0)
- **Tags:** katana, lookdev, nodegraph, katana-9, intermediate
- **Summary:** Parameter PROMOTION in Katana - the step before every Interface Control. Expose a shading node's parameter on the parent NetworkMaterialCreate or NetworkMaterialEdit Parameters tab via the wrench icon > Add to Material Interface (the wrench turns yellow), so artists control a material without entering the network. Works from any shading node inside, including nodes nested in ShadingGroups. Editing in the Material Interface or at the source node updates both, so they never drift. Edit Material Interface Options sets Name, Page and Label - and a full stop in Page nests pages, e.g. Base Layer.Diffuse and Subsurface.
- **File:** tutorials/node-parameters-and-interface-controls.md
- **Related:** Building Materials Using NetworkMaterialCreate (`building-materials-using-networkmaterialcreate.md`), Organizing Shading Networks with ShadingGroup Nodes (`organizing-shading-networks-with-shadinggroup-nodes.md`), Creating Shading Networks (`creating-shading-networks.md`) — share `katana`, `lookdev`, `nodegraph`.
...
- **Summary:** The ShadingGroup node - grouping part of a shading network so only chosen ports appear outside. Creatable ONLY inside a NetworkMaterialCreate. Wire nodes to the fixed INPUT and OUTPUT sidebars by clicking a parameter then the bar, filter long sidebars, and rearrange ports by middle-click dragging (including moving nested ports between groups), with the order reflected on the node after exiting. Port right-click menu jumps to the connected node, deletes or renames - and full stops when renaming create nested pages. Alt+1/2/3 collapse states, X to peek at a collapsed node, and macros for sharing groups.
- **File:** tutorials/organizing-shading-networks-with-shadinggroup-nodes.md
- **Related:** Building Materials Using NetworkMaterialCreate (`building-materials-using-networkmaterialcreate.md`) — the only place a ShadingGroup can be created. Node Parameters and Interface Controls (`node-parameters-and-interface-controls.md`), LiveGroups and LiveShadingGroups (`livegroups-and-liveshadinggroups.md`) — share `katana`, `lookdev`.
...
- **Version:** 9.0 (learn.foundry.com/katana current docs at ingest; release notes whats_new_9.0)
- **Tags:** katana, lookdev, nodegraph, katana-9, advanced
- **Summary:** NetworkMaterialEdit - non-destructive downstream editing of a network material, reading the material.layout attribute at a scene graph location, so a material that arrived through a look file from another department with its original project elsewhere can still be edited fully. Upstream changes propagate. Node striping makes the edit state readable: yellow edited, green new, red disconnected here, padlock disabled because already disconnected upstream, whole network dimmed until first change; NME Filter buttons dim by state. Consolidates the old NetworkMaterialParameterEdit and NetworkMaterialSplice node types.
