...
- [3/3] Nuke Tutorial Series (Flow Paths, FX Integration, Design) (`33-nuke-tutorial-series-flow-paths-fx-integration-design.md`) — shares this same author's stock-energy-effects-library kitbashing philosophy (multiple elements re-mapped and combined as masks for each other, not stacked additively) and several of the same Nukepedia gizmos.
- Shuffle and Channel Management | Nuke Compositing [Beginner / Intermediate] (`shuffle-and-channel-management-nuke-compositing-beginner-intermediate.md`) — shares `channels`, `aovs`; this tutorial's deep-compositing/AOV-heavy workflow depends on exactly the layer-shuffling fundamentals taught there.
- VFX Techniques: Impressions V.S Reality | Nuke Tutorial (`vfx-techniques-impressions-vs-reality-nuke-tutorial.md`) — shares the underlying "combine simple procedural/stock elements rather than fully simulating" philosophy, applied there to a distant-background impression instead of this tutorial's shockwave/lens-flare kitbash.
