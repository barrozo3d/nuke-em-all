...
- [UsdSchemaSet](usdschemaset.md) — shares `katana` + `usd` + `scenegraph`; the node this page lists under **Prims**, documented in depth — applying an API schema (`ShapingAPI`, `ShadowAPI`, `MaterialBindingAPI`) to an existing prim non-destructively, and the `listPosition` list-editing semantics behind it.
- [UsdPrimCreate](usdprimcreate.md) — shares `katana` + `usd` + `scenegraph` + `nodegraph`; the node listed here under **Prims**, in depth — the `primSpec` specifiers (`define` / `class` / `over`), the `primSpecHierarchy` rule that protects lower-layer PrimSpecs, and the seven derived geometry nodes.
- [Importing USD Data](importing-usd-data.md) — shares `katana` + `usd` + `scenegraph` + `nodegraph`; the same five Composition arcs listed here, explained by **purpose** instead of category — payloads for deferred-loadable heavy assets, references for property overrides, inherit to propagate once, specialize when an aspect must never be overridden.
- [Composition Arcs - Understanding LIVRPS](composition-arcs---understanding-livrps.md) — shares `katana` + `usd` + `scenegraph`; the LIVRPS ordering this page names but does not define — Local, Inherits, Variants, References, Payloads, Specializes, strongest first.
- [Loading USD Plug-ins into Katana](loading-usd-plug-ins-into-katana.md) — shares `katana` + `usd` + `katana-9`; the whole native USD node set mapped here becomes available once those plug-ins load — which, on any Katana from 4.5v1, they already have.
