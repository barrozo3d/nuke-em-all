...
- Shuffle and Channel Management | Nuke Compositing [Beginner / Intermediate] (`shuffle-and-channel-management-nuke-compositing-beginner-intermediate.md`) — shares the channel-math/expression fundamentals theme, working directly with individual RGBA channels rather than stock tools.
- Nuke Keying Tutorial | Greenscreen Beginner Concepts (`nuke-keying-tutorial-greenscreen-beginner-concepts.md`) — shares `keying`, `compositing`; that video is the theory/vocabulary primer (key-mixing, garbage/holdout mattes, edge-matte de-spill) underpinning why a real production key needs more than one node — useful to read before this one's hands-on expression technique.
