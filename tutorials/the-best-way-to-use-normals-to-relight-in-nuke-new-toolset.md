...
- The BLUEPRINT for Cinematic Light (VFX) (`the-blueprint-for-cinematic-light-vfx.md`) — shares `relighting`; that video's manual RotateNormals-channel-hunting technique for isolating faces by orientation is the same underlying idea this toolkit's Normal Mixer partially automates.
- A Senior Compositor's Creative CG Workflow REVEALED (`a-senior-compositors-creative-cg-workflow-revealed.md`) — shares `relighting`, `compositing`; that video's glass-shard relighting (using normal passes to add directional highlight/shadow separation against black rather than re-rendering) is the same "use what render passes you already have" philosophy this toolkit is built around.
- Intro to Nuke for 3D Artists - Full VFX Course (`intro-to-nuke-for-3d-artists---full-vfx-course.md`) — that course's normal-pass utility-layer masking and ReLight-node bounce-lighting setup are a more systematic, ground-up version of the RotateNormals-channel-hunting this toolkit partially automates.
