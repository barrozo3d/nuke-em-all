...
- A Senior Compositor's Creative CG Workflow REVEALED (`a-senior-compositors-creative-cg-workflow-revealed.md`) — shares `compositing`, `fx-simulation`; that video's conceptual "2-3 stacked glow layers, never one exponential glow" guidance pairs directly with the actual Shuffle/Grade/Keyer node setup this tutorial demonstrates for building an explosion composite.
- After Effects to Nuke: 1 Hour FREE Course | Compositing in Nuke (`after-effects-to-nuke-1-hour-free-course-compositing-in-nuke.md`) — that beginner course's fire/smoke chapters apply the same Keyer-derived-alpha + Premult + exponential-Glow build taught here, on a single simpler plate rather than a full multi-pass EXR explosion render.
- Intro to Nuke for 3D Artists - Full VFX Course (`intro-to-nuke-for-3d-artists---full-vfx-course.md`) — that full course's beauty-recreation chapters teach the same Shuffle/AOV-plus-recombine + Unpremult/Premult fundamentals this tutorial applies to an explosion render, on a full CG character+environment shot with Cryptomatte and AP_Glow taken further.
- [RenderOutputDefine](renderoutputdefine.md) — shares `channels` + `aovs`; **the render-side origin of these EXRs.** Katana's `exrBitDepth` (16 half for colour, 32 full float for data AOVs), `exrCompression` and `exrType` are what decide the structure of the file opened here.
