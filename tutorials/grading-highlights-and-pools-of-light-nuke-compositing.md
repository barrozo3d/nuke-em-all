...
- Re-lighting Real Footage | Nuke Compositing [Advanced] (`re-lighting-real-footage-nuke-compositing-advanced.md`) — shares `relighting`, `digital-matte-painting`; that video's RealLight/DepthGenerator pass explicitly references this one's pinging-highlights concept as the next realism step.
...
- Nuke Compositing Tutorial: Integration Sketching (`nuke-compositing-tutorial-integration-sketching.md`) — shares `grading`, `relighting`; both are process/methodology lessons on believable light integration by the same presenter.
